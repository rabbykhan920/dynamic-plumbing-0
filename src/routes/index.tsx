...
function CustomCursor() {
  const [cursor, setCursor] = useState({ x: -100, y: -100, label: "", active: false });
  useEffect(() => {
  ...
  return <div className={`custom-cursor ${cursor.active ? "is-active" : ""} ${cursor.label ? "has-label" : ""}`} style={{ transform: `translate3d(${cursor.x}px,${cursor.y}px,0)` }}>{cursor.label}</div>;
}
...
function Header({ scrolled, open, setOpen, active }: { scrolled: boolean; open: boolean; setOpen: (v:boolean)=>void; active:string }) {
  return <motion.header initial={{ y: -80, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 1.1, duration: .7 }} className={`navbar ${scrolled ? "navbar-scrolled" : ""}`}>
    ...
    <a href="#contact" className="gloss-button nav-cta"><span>GET IN TOUCH</span><ArrowUp size={15}/></a>
...
function Hero() {
  ...
  return <section id="home" ref={ref} className="hero">
    <motion.img style={{ y: imageY }} src={heroImage} alt="Modern designer faucet and plumbing in a dark interior" width={1920} height={1080} className="hero-image" />
    <div className="hero-shade"/><div className="ambient-lines" aria-hidden="true" />
    <div className="hero-content">
      <motion.div className="hero-kicker" initial={{ opacity:0 }} animate={{opacity:1}} transition={{delay:1.25}}><span/> PRECISION IN EVERY DETAIL <i>EST. DYNAMIC</i></motion.div>
      <div className="hero-title-wrap">
        <motion.h1 initial={{ y:"105%" }} animate={{ y:0 }} transition={{delay:1.25,duration:.9,ease:[.22,1,.36,1]}}>PLUMBING</motion.h1>
        <motion.h1 className="outline-text" initial={{ y:"105%" }} animate={{ y:0 }} transition={{delay:1.38,duration:.9,ease:[.22,1,.36,1]}}>WITHOUT LIMITS.</motion.h1>
      </div>
      <motion.div className="hero-bottom" initial={{opacity:0,y:24}} animate={{opacity:1,y:0}} transition={{delay:1.7,duration:.7}}>
        <p>Dynamic Plumbing is where we meet your desires and needs in the plumbing industry.</p>
        <div className="hero-actions"><a className="gloss-button primary-button" href="#contact"><span>GET IN TOUCH</span><ArrowUp size={18}/></a><a className="text-link" href="#services">EXPLORE SERVICES <ArrowDown size={16}/></a></div>
      </motion.div>
    </div>
    <div className="hero-badge"><span>01</span><p>FORM<br/>MEETS<br/>FUNCTION</p></div>
  </section>;
}
...
function Reels() {
  return <section id="reels" className="reels section-pad">
    ...
    <div className="reel-track">{[1,2,3].map((n,i)=><motion.div key={n} className={`reel-card reel-${n}`} data-cursor="REEL" initial={{opacity:0,y:70,rotate:i===0?-5:i===2?5:0}} whileInView={{opacity:1,y:0,rotate:i===0?-3:i===2?3:0}} viewport={{once:true}} transition={{delay:i*.13,duration:.8}} whileHover={{y:-16,rotate:0}}>
...
function Gallery() {
  ...
  return <section id="gallery" className="gallery section-pad">
    ...
    <div className="orbit-gallery" data-cursor="EXPLORE">{galleryImages.map((image,i)=>{
...
function Contact() {
  return <section id="contact" className="contact section-pad"><div className="contact-glow"/><motion.div {...reveal}><SectionLabel n="07">START A CONVERSATION</SectionLabel><h2>LET'S MAKE YOUR NEXT<br/>PLUMBING PROJECT <span>DYNAMIC.</span></h2><div className="contact-actions"><a className="gloss-button primary-button" href={phoneHref}><Phone size={17}/><span>CALL NOW</span><ArrowUp/></a><a className="gloss-button" href={emailHref}><Mail size={17}/><span>SEND EMAIL</span><ArrowUp/></a><a className="gloss-button" href={instagramHref} target="_blank" rel="noreferrer"><Instagram size={17}/><span>VISIT INSTAGRAM</span><ArrowUp/></a></div></motion.div></section>;
}
